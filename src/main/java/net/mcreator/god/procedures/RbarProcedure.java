...
public class RbarProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RbarProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RbarProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(2)) {
			RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "barcasing" + ".png")));
			renderTexture(28, 13, 1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			renderTexture(28, 25, 1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "barcasing3" + ".png")));
			renderTexture(28, 7, 1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			release();
		}
	}
}

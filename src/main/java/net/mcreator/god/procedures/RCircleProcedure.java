...
public class RCircleProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RCircleProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RCircleProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		double q3 = 0;
		if (target(2)) {
			RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "cirlce" + ".png")));
			renderTexture(17, 6, 0, 0, 1, 200 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			release();
		}
	}
}

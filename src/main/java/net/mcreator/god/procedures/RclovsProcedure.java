...
public class RclovsProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RclovsProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RclovsProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			SELECT = (player.getCapability(GodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new GodModVariables.PlayerVariables())).SELECT;
			if (SELECT == 1) {
				RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "heart2" + ".png")));
			} else if (SELECT == 2) {
				RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "spade2" + ".png")));
			} else if (SELECT == 3) {
				RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "clover2" + ".png")));
			} else if (SELECT == 4) {
				RenderSystem.setShaderTexture(0, new ResourceLocation(("god" + ":textures/" + "diamond2" + ".png")));
			}
			renderTexture(25, 15, -1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			release();
		}
	}
}
